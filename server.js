...
// 1. Serve uploaded files statically so the frontend can display them
const UPLOAD_ROOT =
...
// FILE UPLOAD
app.use("/api/properties/assets/:propertyId", assetsRouter);
...
app.put("/api/properties/negotiable/:id", verifyApiKey, (req, res) => {
  ...
  db.run(sql, [isNegotiable ? 1 : 0, id], function (err) {
    ...
    res.json({
      success: true,
      message: `Property ${id} negotiable status updated to ${isNegotiable}`,
    });
  });
});
...
app.put("/api/properties/archive/:id", verifyApiKey, (req, res) => {
  ...
  db.run(sql, [isArchived ? 1 : 0, id], function (err) {
    ...
    res.json({
      success: true,
      message: `Property ${id} archive status updated to ${isArchived}`,
    });
  });
});
...
app.delete("/api/properties", verifyApiKey, (req, res) => {
  ...
  if (!ids || !Array.isArray(ids) || ids.length === 0) {
    return res
      .status(400)
      .json({ success: false, error: "Invalid or missing property IDs" });
  }
  ...
  db.run(sql, ids, function (err) {
    ...
    res.json({
      success: true,
      message: `${this.changes} properties deleted successfully`,
    });
  });
});
